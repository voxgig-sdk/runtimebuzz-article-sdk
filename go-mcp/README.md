# runtimebuzz-article-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RuntimebuzzArticle SDK as
two agent tools — `runtimebuzz-article_list` and `runtimebuzz-article_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/runtimebuzz-article-mcp)
make build

# 2. Provide credentials via the environment
export RUNTIMEBUZZ_ARTICLE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user runtimebuzz-article \
  -- /absolute/path/to/runtimebuzz-article-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./runtimebuzz-article-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// runtimebuzz-article_list: first page of records
{ "entity": "read_finder_index" }
{ "entity": "read_finder_index", "query": { } }

// runtimebuzz-article_load: one record by id
{ "entity": "read_finder_index", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/runtimebuzz-article-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user runtimebuzz-article \
     -- "$PWD"/dist/*/runtimebuzz-article-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `runtimebuzz-article_list` and `runtimebuzz-article_load` tools now appear
   in new sessions. Ask the agent to *"list read_finder_index using runtimebuzz-article"*
   and it calls `runtimebuzz-article_list` with `{"entity":"read_finder_index"}`.
...
```sh
export RUNTIMEBUZZ_ARTICLE_APIKEY=sk_live_xxx            # API key
export RUNTIMEBUZZ_ARTICLE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `runtimebuzz-article_list` tool
...
### Call the `runtimebuzz-article_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `runtimebuzz-article_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `runtimebuzz-article_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RUNTIMEBUZZ_ARTICLE_APIKEY` | API key sent with every request. |
| `RUNTIMEBUZZ_ARTICLE_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

read_finder_index | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./runtimebuzz-article-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"runtimebuzz-article_load","arguments":{"entity":"read_finder_index","query":{"id":1}}}}'
```
