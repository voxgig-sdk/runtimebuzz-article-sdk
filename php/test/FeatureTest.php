...
// RuntimebuzzArticle SDK feature test
//
...
class FtCtrl extends RuntimebuzzArticleControl
{
...
class FtClient
{
    ...
    private RuntimebuzzArticleUtility $utility;

    public function __construct(RuntimebuzzArticleUtility $utility, array $options)
    {
    ...
    public function get_utility(): RuntimebuzzArticleUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RuntimebuzzArticleContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RuntimebuzzArticleContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RuntimebuzzArticleContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RuntimebuzzArticleConfig::make_feature($name);
    ...
    private static function buildUrl(RuntimebuzzArticleSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RuntimebuzzArticleError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RuntimebuzzArticleResponse($fetched);
        }
    ...
    private function fail(RuntimebuzzArticleContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RuntimebuzzArticleContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
