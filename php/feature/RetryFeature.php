...
// RuntimebuzzArticle SDK retry feature
...
class RuntimebuzzArticleRetryFeature extends RuntimebuzzArticleBaseFeature
{
    ...
    public function init(RuntimebuzzArticleContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RuntimebuzzArticleContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RuntimebuzzArticleContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RuntimebuzzArticleContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
