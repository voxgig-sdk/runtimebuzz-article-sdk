...
// RuntimebuzzArticle SDK ReadFinderIndex entity
...
class ReadFinderIndexEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ReadFinderIndexEntity($this->_client, $opts);
    }

    /**
     * @param ReadFinderIndex|array $args ReadFinderIndex data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RuntimebuzzArticleHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ReadFinderIndex|array The current ReadFinderIndex data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ReadFinderIndex fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RuntimebuzzArticleHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ReadFinderIndex fields).
     */
    ...
    /**
     * Load a single ReadFinderIndex.
     *
     * @param ReadFinderIndexLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ReadFinderIndexLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ReadFinderIndex|array The loaded ReadFinderIndex as an assoc-array at the
     *   SDK boundary; throws RuntimebuzzArticleError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RuntimebuzzArticleHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
