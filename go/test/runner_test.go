...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("RUNTIMEBUZZARTICLE_TEST_LIVE") == "TRUE" ||
		os.Getenv("RUNTIMEBUZZARTICLE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("RUNTIMEBUZZARTICLE_TEST_EXPLAIN"); explain != "" {
