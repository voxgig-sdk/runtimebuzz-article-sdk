...
require_relative 'RuntimebuzzArticle_types'
...
class RuntimebuzzArticleSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RuntimebuzzArticleHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RuntimebuzzArticleFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RuntimebuzzArticleUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = RuntimebuzzArticleSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RuntimebuzzArticleError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RuntimebuzzArticleError => err
    ...
    if fetched.is_a?(Hash)
      status = RuntimebuzzArticleHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RuntimebuzzArticleError.new(
        "graphql_error", "RuntimebuzzArticleSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.ReadFinderIndex.list / client.ReadFinderIndex.load({ "id" => ... })
  def ReadFinderIndex(data = nil)
    require_relative 'entity/read_finder_index_entity'
    ReadFinderIndexEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RuntimebuzzArticleSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
