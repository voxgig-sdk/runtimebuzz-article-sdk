...
class ReadFinderIndexEntity
  ...
  def make
    opts = @_entopts.dup
    ReadFinderIndexEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ReadFinderIndex, Hash] the current ReadFinderIndex data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ReadFinderIndex fields)
  def match_get
  ...
  #
  # @param reqmatch [ReadFinderIndexLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ReadFinderIndex.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ReadFinderIndex, Hash] the loaded ReadFinderIndex; raises RuntimebuzzArticleError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RuntimebuzzArticleHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
