...
const SDK_NAME = 'RuntimebuzzArticleSDK'
...
const TEST_SEED = {"entity":{"read_finder_index":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+RuntimebuzzArticleSDK\b/.test(code) ||
    /\bRuntimebuzzArticleSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RuntimebuzzArticleSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RuntimebuzzArticleSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
