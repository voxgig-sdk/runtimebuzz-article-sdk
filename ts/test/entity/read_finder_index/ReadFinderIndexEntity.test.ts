...
describe('ReadFinderIndexEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when RUNTIMEBUZZARTICLE_TEST_LIVE=TRUE.
  afterEach(liveDelay('RUNTIMEBUZZARTICLE_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'read_finder_index.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set RUNTIMEBUZZ_ARTICLE_TEST_READ_FINDER_INDEX_ENTID JSON to run live')
      return
    }
    ...
    const read_finder_index_ref01_data_dt0 = await read_finder_index_ref01_ent.load(read_finder_index_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['RUNTIMEBUZZ_ARTICLE_TEST_READ_FINDER_INDEX_ENTID']
