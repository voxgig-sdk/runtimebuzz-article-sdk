    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RUNTIMEBUZZARTICLE_TEST_SEARCH_ENTID JSON to run live")
        client = setup["client"]
...
def _search_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "RUNTIMEBUZZARTICLE_TEST_SEARCH_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "RUNTIMEBUZZARTICLE_TEST_SEARCH_ENTID": idmap,
        "RUNTIMEBUZZARTICLE_TEST_LIVE": "FALSE",
        "RUNTIMEBUZZARTICLE_TEST_EXPLAIN": "FALSE",
    })

    idmap_resolved = helpers.to_map(
        env.get("RUNTIMEBUZZARTICLE_TEST_SEARCH_ENTID"))
