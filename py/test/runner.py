# RuntimebuzzArticle SDK test runner
...
class RuntimebuzzArticleTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RuntimebuzzArticleTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RuntimebuzzArticleTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RuntimebuzzArticleTestRunner.getenv("RUNTIMEBUZZ_ARTICLE_TEST_LIVE")
        override = RuntimebuzzArticleTestRunner.getenv("RUNTIMEBUZZ_ARTICLE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RuntimebuzzArticleTestRunner.getenv(key)
        ...
        explain = RuntimebuzzArticleTestRunner.getenv("RUNTIMEBUZZ_ARTICLE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RuntimebuzzArticleTestRunner._test_control is not None:
            return RuntimebuzzArticleTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RuntimebuzzArticleTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RuntimebuzzArticleTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RuntimebuzzArticleTestRunner.load_test_control()
...
def load_env_local():
    RuntimebuzzArticleTestRunner.load_env_local()


def env_override(m):
    return RuntimebuzzArticleTestRunner.env_override(m)


def entity_data(v):
    return RuntimebuzzArticleTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RuntimebuzzArticleTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RuntimebuzzArticleTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RuntimebuzzArticleTestRunner.load_test_control()


def live_delay_ms():
    return RuntimebuzzArticleTestRunner.live_delay_ms()
