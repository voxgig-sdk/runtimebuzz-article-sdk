...
module RuntimebuzzArticleUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RuntimebuzzArticleResponse.new({})
    ...
    if fetch_err
      ...
      response = RuntimebuzzArticleResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RuntimebuzzArticleResponse.new(fetched)
    else
