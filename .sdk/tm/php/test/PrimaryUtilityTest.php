...
// RuntimebuzzArticle SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        RuntimebuzzArticleSDK $client,
        RuntimebuzzArticleUtility $utility
    ): RuntimebuzzArticleContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new RuntimebuzzArticleContext($ctxmap, null);
    ...
    private static function fixctx(RuntimebuzzArticleContext $ctx, RuntimebuzzArticleSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?RuntimebuzzArticleError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RuntimebuzzArticleError($code, $msg);
    }
    ...
    {
        $client = RuntimebuzzArticleSDK::test(null, null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new RuntimebuzzArticleResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new RuntimebuzzArticleBaseFeature();
    ...
    {
        $hook_client = RuntimebuzzArticleSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends RuntimebuzzArticleBaseFeature {
            public $hook_fn = null;
            public function TestHook(RuntimebuzzArticleContext $ctx): void
            {
    ...
    {
        $init_client = RuntimebuzzArticleSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends RuntimebuzzArticleBaseFeature {
            public $init_fn = null;
            ...
            public function init(RuntimebuzzArticleContext $ctx, array $options): void
            {
    ...
    {
        $calls = [];
        $live_client = new RuntimebuzzArticleSDK([
    ...
    {
        $blocked_client = new RuntimebuzzArticleSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new RuntimebuzzArticleSpec([
            'base' => 'http://localhost:8080',
            'prefix' => '/api',
            'path' => 'items/{id}',
            'suffix' => '',
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new RuntimebuzzArticleSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = RuntimebuzzArticleSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new RuntimebuzzArticleOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = RuntimebuzzArticleSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof RuntimebuzzArticleError) ? $result->err->msg : (string)$result->err;
