// Command RuntimebuzzArticle-cli is a CLI + REPL for the RuntimebuzzArticle SDK,
...
const prompt = "runtimebuzz-article"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "read_finder_index search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RUNTIMEBUZZ_ARTICLE_APIKEY (api key), RUNTIMEBUZZ_ARTICLE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RUNTIMEBUZZ_ARTICLE_APIKEY carries the API key and
	// RUNTIMEBUZZ_ARTICLE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RUNTIMEBUZZ_ARTICLE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RUNTIMEBUZZ_ARTICLE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRuntimebuzzArticleSDK(opts)
