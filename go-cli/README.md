# runtimebuzz-article-cli

boru-driven command-line client **and** interactive REPL for the RuntimebuzzArticle
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/runtimebuzz-article-cli)
make build

# 2. See usage (words, entities, env vars)
./runtimebuzz-article-cli --help

# 3. Provide credentials once, via the environment
export RUNTIMEBUZZ_ARTICLE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./runtimebuzz-article-cli load 1 read_finder_index            # {id:1} shorthand
./runtimebuzz-article-cli load '{id:1}' read_finder_index       # explicit match map

# 5. Override the API base URL for a single call
RUNTIMEBUZZ_ARTICLE_BASE=https://api.example.com ./runtimebuzz-article-cli load 1 read_finder_index

# 6. No arguments -> interactive REPL
./runtimebuzz-article-cli
runtimebuzz-article> load 1 read_finder_index
runtimebuzz-article> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/runtimebuzz-article-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/runtimebuzz-article-cli load 1 read_finder_index
   ```
...
```sh
./runtimebuzz-article-cli load 1 read_finder_index          # scalar shorthand for {id:1}
./runtimebuzz-article-cli load '{id:1}' read_finder_index     # explicit match map
```
...
./runtimebuzz-article-cli load 1 read_finder_index
```
...
Run with no arguments to open a REPL (prompt `runtimebuzz-article>`). Each line is
evaluated as its own boru expression:

```text
$ ./runtimebuzz-article-cli
runtimebuzz-article> load 1 read_finder_index
runtimebuzz-article> /help
runtimebuzz-article> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `read_finder_index`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/runtimebuzz-article-cli`. |
...
(`./runtimebuzz-article-cli <expr>`) and interactively (the REPL), and expressions compose the
